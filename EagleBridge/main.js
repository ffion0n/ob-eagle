...
function startServer(libraryPath, port) {
  ...
  server = http.createServer((req, res) => {
    ...
    const imagesRoot = path.join(libraryPath, "images") + path.sep;
...
function getInfoExtFromLocalHostUrl(urlStr) {
  ...
  const id = match[1];
  const cacheKey = `${currentLibraryPath}::${id}`;
  const cached = infoExtCacheForEmbed.get(cacheKey);
  if (cached) {
    return cached;
  }
  try {
    const metadataPath = path.join(currentLibraryPath, "images", `${id}.info`, "metadata.json");
...
function getInfoExtFromLocalHostUrlAsync(urlStr, maxAttempts = 10) {
  ...
  const id = match[1];
  const key = `${currentLibraryPath || url.origin}::${id}`;
      ...
      if (embedManager.shouldEmbed(this.url)) {
        ...
        if (ext && audioExtsForEmbed.has(ext)) {
          ...
          getInfoExtFromLocalHostUrlAsync(this.url).then((ext2) => {
            ...
            this.container.className = "eagle-embed-placeholder cm-embed-block";
            this.container.textContent = "\u65E0\u6CD5\u5F97\u5230\u9644\u4EF6\u4FE1\u606F\uFF0C\u70B9\u51FB\u6253\u5F00\u94FE\u63A5\u3002";
          });
        }
      } else {
